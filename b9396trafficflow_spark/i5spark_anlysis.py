...
def data_anlysis(inputFile):
    ...
    while True:
        ...
        topTraffics = hiveCtx.sql(
            "SELECT placeid, time, car_type_small, car_type_middle, car_type_large, car_total_num, car_speeds FROM traffic ORDER BY time LIMIT 10"
        )
        show1 = colored("\n 1. According to lastest time order:", "blue", attrs=['reverse', 'blink'])
        print("#" * 20, show1, topTraffics.collect(),' record count:', len( topTraffics.collect()))
        ...
        for singlelist in topTrafficText.collect():
            ...
            show3 = colored("3. Traffic flow is:", "red", attrs=['reverse', 'blink'])
