...
behavior_df = spark.read.csv(
    "data/user_behavior_data.csv", header=True, inferSchema=True
)
...
plt.legend(
    title=current_legend.get_title().get_text(),
    labels=[t.get_text() for t in current_legend.texts],
    loc=current_legend._loc,
    prop=font,
)
...
user_recs_with_profile = user_recs_with_profile.withColumn(
    "label", user_recs_with_profile["gender"].isin(["Male"]).cast("double")
)
...
lr = LogisticRegression(
    featuresCol="features", labelCol="label", regParam=0.1, elasticNetParam=0.5
)
...
new_user_recs_with_profile = new_user_recs_with_profile.withColumn(
    "label", new_user_recs_with_profile["gender"].isin(["Male"]).cast("double")
)
...
item_df = spark.createDataFrame(
    item_data, schema=["itemId", "itemName", "itemCategory"]
)
...
user_recs_exploded = user_recs_with_profile.select(
    "userId", explode("recommendations").alias("recommendation")
)

# 提取项目ID和评分
user_recs_exploded = user_recs_exploded.select(
    "userId", "recommendation.itemId", "recommendation.rating"
)
...
# 添加聚类标签到用户画像数据中
user_clusters = (
    kmeans_model.transform(user_recs_with_profile)
    .select("userId", "prediction")
    .withColumnRenamed("prediction", "cluster")
)

# 将聚类标签连接到推荐结果数据中
user_recs_with_profile = user_recs_with_profile.join(
    user_clusters, on="userId", how="left"
)
...
# 使用用户画像和推荐结果生成新用户的标签
new_user_clusters = (
    kmeans_model.transform(new_user_recs_with_profile.select("features"))
    .select("prediction")
    .withColumnRenamed("prediction", "cluster")
)
new_user_recs_with_profile = new_user_recs_with_profile.join(
    new_user_clusters, how="left"
)
...
# 绘制用户画像分布图，按聚类标签分别显示
sns.scatterplot(
    data=user_recs_with_profile.toPandas(), x="longitude", y="latitude", hue="cluster"
)
plt.show()
...
# 计算预测精度
test_accuracy = gbt_predictions.filter(
    gbt_predictions.label == gbt_predictions.prediction
...
new_user_profile_df = pd.DataFrame(
    {
        "userId": [5001],
        "age": [28],
        "gender": ["Female"],
