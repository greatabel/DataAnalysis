...
def dme_dme_position(known_positions, distances):
    ...
    for i in range(1, len(known_positions)):
        ...
        b[i - 1] = distances[i] ** 2 - distances[0] ** 2 - np.sum((known_positions[i] - known_positions[0]) ** 2)
...
def plot_trajectory(actual_positions, estimated_positions, mean_offset, animate=False, bg_image=None):
    if animate:
        ...
        def animate(i):
            ...
            if bg_image is not None:
                ...
                min_lon = min(np.min(actual_positions[:, 0]), np.min(dme_stations[:, 0]))
                max_lon = max(np.max(actual_positions[:, 0]), np.max(dme_stations[:, 0]))
                min_lat = min(np.min(actual_positions[:, 1]), np.min(dme_stations[:, 1]))
                max_lat = max(np.max(actual_positions[:, 1]), np.max(dme_stations[:, 1]))
                ...
                img_extent = [min_lon - lon_margin, max_lon + lon_margin, min_lat - lat_margin, max_lat + lat_margin]
                ...
                for station in dme_stations:
                    plt.plot([drone_position[0], station[0]], [drone_position[1], station[1]], color='blue', linestyle=':', linewidth=0.5)
                # Add lines to represent predicted communication with the DME stations
                estimated_position = estimated_positions[i] + mean_offset
                for station in dme_stations:
                    plt.plot([estimated_position[0], station[0]], [estimated_position[1], station[1]], color='yellow', linestyle='--', linewidth=0.5)
            ...
            plt.plot(actual_positions[:i, 0], actual_positions[:i, 1], label='Actual trajectory')
            plt.plot(corrected_estimated_positions[:, 0], corrected_estimated_positions[:, 1], label='Corrected estimated trajectory', linestyle='--')
            plt.scatter(dme_stations[:, 0], dme_stations[:, 1], marker='^', color='red', label='DME stations')

            for i, station in enumerate(dme_stations):
                plt.annotate(f"DME {i+1}", (station[0], station[1]), textcoords="offset points", xytext=(-15, 5), fontsize=9, color='darkred')
            ...
            plt.ylabel('Latitude')
            plt.legend()
            plt.title('Drone flight trajectory using DME/DME navigation with mean offset compensation')
            plt.grid()

        # Call the animator
        ani = animation.FuncAnimation(plt.figure(figsize=(10, 8)), animate, frames=len(actual_positions), interval=1)
...
if __name__ == "__main__":
    actual_positions, estimated_positions, mean_offset = fly_drone(waypoints, dme_stations)
    plot_trajectory(actual_positions, estimated_positions, mean_offset, animate=True, bg_image="bg.png")
